...
class DbHandler {	
    ...
    public function getAllProperties() {
        $stmt = $this->pdo->prepare("SELECT p.*,pl.listing, pt.type, a_s.approval_status, u.*  FROM properties p INNER JOIN property_listing pl ON p.property_listing = pl.id LEFT JOIN users u ON p.manager = u.userId INNER JOIN property_type pt ON p.property_type = pt.id INNER JOIN approval_status a_s ON p.approval = a_s.id ORDER BY p.id DESC");
        $stmt->execute();
    ...
    public function getPublishedProperties() {
        $stmt = $this->pdo->prepare("SELECT p.*,pl.listing, pt.type, a_s.approval_status, u.*  FROM properties p INNER JOIN property_listing pl ON p.property_listing = pl.id INNER JOIN users u ON p.manager = u.userId INNER JOIN property_type pt ON p.property_type = pt.id INNER JOIN approval_status a_s ON p.approval = a_s.id WHERE p.approval = ? ORDER BY p.id DESC");
        $stmt->execute([1]);
		$properties = $stmt->fetchAll();
		if(!$properties) exit('No rows');
		return $properties;
	...
	public function getProperty($propertyId){
		$stmt = $this->pdo->prepare("SELECT p.*,pl.listing, pt.type, a_s.approval_status, u.*  FROM properties p INNER JOIN property_listing pl ON p.property_listing = pl.id INNER JOIN users u ON p.manager = u.userId INNER JOIN property_type pt ON p.property_type = pt.id INNER JOIN approval_status a_s ON p.approval = a_s.id WHERE p.id = ? ");
	...
	public function check_login($uname, $pword) {
		...
		if(!$result){
			...
			$_SESSION["fail"] = 1;
		  	...
		  	$stmt1 = $this->pdo->prepare("SELECT * FROM users WHERE username = ? AND password = ?");
	...
	* Save technician
	*/
	public function save_technician($name,$phone,$location,$service, $photoUrl,$info, $rating,$experience,$clientele) {
		$stmt = $this->pdo->prepare("INSERT INTO service_providers (name, phone,info,rating, photoUrl,locality,serviceId,experience,clients) VALUES (?, ?, ?, ?, ?, ? ,?, ?, ?)");
		$stmt->execute(array($name, $phone, $info, $rating, $photoUrl, $location, $service, $experience,$clientele));
		return($this->pdo->lastInsertId());
		$stmt = null;
	}
	...
	* Save user
	*/
	public function save_user($name,$phone,$email,$photoUrl,$uname, $password, $utype) {		
		$stmt = $this->pdo->prepare("INSERT INTO users (username,password,name, phone_number,email_address, photo_url,user_type) VALUES (?, ?, ?, ?, ?, ?, ?)");
		$stmt->execute(array($uname, $password, $name, $phone, $email, $photoUrl, $utype));
		return($this->pdo->lastInsertId());
		$stmt = null;
	}
	...
	public function getUserType(){
		$stmt = $this->pdo->prepare("SELECT * FROM userTypes");
        $stmt->execute();
		$types = $stmt->fetchAll();
     ...
     */
	public function edit_technician($providerId,$name,$phone,$info, $rating, $location,$service, $photoUrl,$experience, $clientele){
		$stmt = $this->pdo->prepare("UPDATE service_providers SET name = ?, phone = ?, info=?, rating= ?, photoUrl = ?, locality = ?, serviceId = ?, experience =?, clients = ? WHERE id = ? ");
        $stmt->execute([$name,$phone,$info, $rating,$photoUrl,$location,$service,$experience, $clientele,$providerId]);
		return $stmt -> rowCount();
		$stmt = null;		
	}
     ...
     * Edit User 
     */
	public function edit_user($userId,$uname,$name,$phone,$email, $photoUrl,$utype){
		$stmt = $this->pdo->prepare("UPDATE users SET username = ?, name = ?, phone_number = ?, email_address = ?, photo_url = ?, user_type = ? WHERE userId = ? ");
        $stmt->execute([$uname,$name,$phone,$email,$photoUrl,$utype,$userId]);
		return $stmt -> rowCount();
		$stmt = null;		
	}
	...
	public function getUserUrl($userId){
		$stmt = $this->pdo->prepare("SELECT photo_url FROM users WHERE userId = ? ");
        $stmt->execute([$userId]);
		$url = $stmt->fetchColumn();
		//if(!$url) exit('No rows');
		return $url;
     ...
     * Provider url 
     */
	public function getProfileUrl($providerId){
		$stmt = $this->pdo->prepare("SELECT photoUrl FROM service_providers WHERE id = ? ");
        $stmt->execute([$providerId]);
		$url = $stmt->fetchColumn();
		//if(!$url) exit('No rows');
		return $url;
     ...
     * Get notation 
     */
	public function getNotation($listing){
		$stmt = $this->pdo->prepare("SELECT property_notation FROM property_notation WHERE property_type = ? ");
        $stmt->execute([$listing]);
		$notation = $stmt->fetchColumn();
		if(!$notation) exit('No rows');
		return $notation;
		$stmt = null;		
	}
}

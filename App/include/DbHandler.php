...
class DbHandler {	
    ...
    public function getAllProperties($listing) {
		$approval = 1;
        $stmt = $this->pdo->prepare("SELECT p.*, pt.type, u.*, n.property_notation FROM properties p INNER JOIN users u ON p.manager = u.userId INNER JOIN property_type pt ON p.property_type = pt.id INNER JOIN property_notation n ON p.property_listing = n.property_type WHERE p.property_listing = ? AND p.approval = ?  ORDER BY p.id DESC");

        $stmt->execute(array($listing, $approval));

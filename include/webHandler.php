...
class DbHandler {	
    ...
    public function getAllProperties() {
        $stmt = $this->pdo->prepare("SELECT p.*,pl.listing, pt.type, a_s.approval_status, u.*  FROM properties p INNER JOIN property_listing pl ON p.property_listing = pl.id INNER JOIN users u ON p.manager = u.userId INNER JOIN property_type pt ON p.property_type = pt.id INNER JOIN approval_status a_s ON p.approval = a_s.id ORDER BY p.id DESC");
        $stmt->execute();
	...
	* Save technician
	*/
	public function save_technician($name,$phone,$location,$service, $photoUrl,$info, $rating) {
		$stmt = $this->pdo->prepare("INSERT INTO service_providers (name, phone,info,rating photoUrl,locality,serviceId) VALUES (?, ?, ?, ?, ?, ? ,?)");
		$stmt->execute(array($name, $phone, $info, $rating, $photoUrl, $location, $service));
		return($this->pdo->lastInsertId());
		$stmt = null;
	}
	...
	*/
     ...
     */
	public function edit_technician($providerId,$name,$phone,$info, $rating, $location,$service, $photoUrl){
		$stmt = $this->pdo->prepare("UPDATE service_providers SET name = ?, phone = ?, info=?, rating= ?, photoUrl = ?, locality = ?, serviceId = ? WHERE id = ? ");
        $stmt->execute([$name,$phone,$info, $rating,$photoUrl,$location,$service,$providerId]);
		return $stmt -> rowCount();
		$stmt = null;		
	}
